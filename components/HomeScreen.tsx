  ...
  const fetchTeams = async () => {
    ...
    try {
      ...
      const filteredTeams = (teamData ?? []).filter((team: any) => {
        ...
        return team.user_email === user.email
      })
      ...
      const groupedMembers = membersData.reduce((acc: Record<string, TeamMember[]>, member: any) => {
        const key = member.team_id
        if (!acc[key]) acc[key] = []
        acc[key].push({
          id: member.id,
          name: member.name,
      ...
      const sanitizedTeams: Team[] = filteredTeams.map((team: any) => ({
        id: team.id,
        name: team.name,
  ...
  try {
    ...
    for (const member of updatedTeam.members) {
      if (member.id && existingMemberIds.includes(member.id)) {
        const { error: updateError } = await supabase
          .from("team_members")
          .update({ name: member.name, score: member.score, position: member.position ?? "midfield" })
          .eq("id", member.id)
    ...
    if (newMembers.length > 0) {
      ...
      updatedTeam.members = updatedTeam.members.map((m) => {
        const dbMatch = insertedMembers.find(
          (im) =>
            im.name === m.name &&
            im.score === m.score &&
            (im.position ?? "midfield") === (m.position ?? "midfield")
        )
